...
TEST_F(EgoMotionModelTest, test_egoMotionModel_Covariances)
{
  ...
  kf::Matrix<DIM_X, DIM_X> const matQk{
      egoMotionModel.getProcessNoiseCov(vecX, vecU)};
  kf::Matrix<DIM_X, DIM_X> const matUk{
      egoMotionModel.getInputNoiseCov(vecX, vecU)};

  kf::Matrix<DIM_X, DIM_X> const matFk{
      egoMotionModel.getJacobianFk(vecX, vecU)};
  kf::Matrix<DIM_X, DIM_U> const matBk{
      egoMotionModel.getJacobianBk(vecX, vecU)};

  kf::Matrix<DIM_X, DIM_X> matQ;
  matQ << qX, 0.0F, 0.0F, 0.0F, qY, 0.0F, 0.0F, 0.0F, qTheta;

  kf::Matrix<DIM_U, DIM_U> matU;
  matU << uDeltaDist, 0.0F, 0.0F, uDeltaYaw;
...
TEST_F(EgoMotionModelTest, test_egoMotionModel_ForwardReverseMoves)
{
  ...
  EXPECT_NEAR(vecXn[0],
              vecX[0] + vecU[0] * std::cos(vecX[2] + (vecU[1] / 2.0F)),
              0.0001F);
  EXPECT_NEAR(vecXn[1],
              vecX[1] + vecU[0] * std::sin(vecX[2] + (vecU[1] / 2.0F)),
              0.0001F);
...
TEST_F(EgoMotionModelTest, test_egoMotionModel_EKF_ForwardReverseMoves)
{
  ...
  EXPECT_NEAR(kalmanFilter.vecX()[0],
              vecX[0] + vecU[0] * std::cos(vecX[2] + (vecU[1] / 2.0F)),
              0.0001F);
  EXPECT_NEAR(kalmanFilter.vecX()[1],
              vecX[1] + vecU[0] * std::sin(vecX[2] + (vecU[1] / 2.0F)),
              0.0001F);
