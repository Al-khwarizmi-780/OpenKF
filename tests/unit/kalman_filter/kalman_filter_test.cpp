...
class KalmanFilterTest : public testing::Test
{
 public:
  ...
  static kf::Vector<DIM_Z_EKF> covertCartesian2Polar(
      const kf::Vector<DIM_X_EKF>& cartesian)
  {
  ...
  static kf::Matrix<DIM_Z_EKF, DIM_Z_EKF> calculateJacobianMatrix(
      const kf::Vector<DIM_X_EKF>& vecX)
  {
...
TEST_F(KalmanFilterTest, test_predictLKF)
{
  ...
  Q(0, 0) = (Q11 * SAMPLE_TIME_SEC) +
            (Q22 * (std::pow(SAMPLE_TIME_SEC, 3.0F) / 3.0F));
...
TEST_F(KalmanFilterTest, test_correctEKF)
{
  ...
  kf::Matrix<DIM_Z_EKF, DIM_X_EKF> matHj{
      calculateJacobianMatrix(m_ekf.vecX())};  // jacobian matrix Hj
