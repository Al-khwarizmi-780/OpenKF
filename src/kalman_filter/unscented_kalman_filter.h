...
class UnscentedKalmanFilter : public KalmanFilter<DIM_X, DIM_Z>
{
 ...
 private:
  ...
  /// @param kappa design scaling parameter for sigma points selection
  ///
  Matrix<DIM_A, SIGMA_DIM> calculateSigmaPoints(
      const Vector<DIM_A>& vecXa, const Matrix<DIM_A, DIM_A>& matPa,
  ...
  void calculateWeightedMeanAndCovariance(
      const Matrix<STATE_DIM, SIGMA_DIM>& sigmaX, Vector<STATE_DIM>& vecX,
