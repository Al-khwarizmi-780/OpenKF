...
class SquareRootUKF : public KalmanFilter<DIM_X, DIM_Z>
{
 public:
  ...
  void correctSRUKF(MeasurementModelCallback measurementModelFunc,
 ...
 private:
  ...
  /// @param kappa design scaling parameter for sigma points selection
  ///
  Matrix<DIM_X, SIGMA_DIM> calculateSigmaPoints(
      const Vector<DIM_X>& vecXk, const Matrix<DIM_X, DIM_X>& matSk,
  ...
  Matrix<DIM, DIM> updatePredictedCovariance(
      const Matrix<DIM, SIGMA_DIM>& matSigmaX, const Vector<DIM>& meanX,
  ...
  Matrix<SIGMA_DIM + DIM_X - 1, DIM_X> buildCompoundMatrix(
      const Matrix<DIM_X, SIGMA_DIM>& matSigmaX, const Vector<DIM_X>& meanX,
